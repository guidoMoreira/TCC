...
#Sistema para scroll mover linhas para cima e para baixo, criando novas linhas caso passe do último limite
func _process(delta):
...
#Adiciona linha ao final da última linha, deleta detector de posição da última linha anterior e atualiza o contador de posição
func _new_line():
