...
#Move gráfico para posição certa, mas remove se janela de salvar iamgem estiver na frente
func _process(delta):
	#graf_movel.rect_position.x = -144 - scroll_bar.value*40
	if visible and !$FileSaveGraf.visible:
		$ViewportContainer.rect_position.x = -133
		$ViewportContainer/Viewport/movel.rect_position.x = 80- scroll_bar.value*40
		$ViewportContainer.rect_position.y = -91
	else:
		$ViewportContainer.rect_position = Vector2(1018,991)

#Exibir gráfico na posição certa
func open_graf():
	$ViewportContainer.rect_position = Vector2(-133,-91)
	visible = true

#Esconder gráfico em outra posição
func close_graf():
	$ViewportContainer.rect_position = Vector2(1018,991)
	$FileSaveGraf.visible = false
	visible = false

#Cria gráfico baseado nos dados do histórico de desempenho
func load_graf(historico):
	viewport.size.x = min(len(historico["historico"])*40 + 72, 1100) # 16384
	for h in historico["historico"]:
		...
		if h[3] == "Acerto":
			print(h)
			print(pt1.rect_global_position)
			print(pt2.rect_global_position)
...
#Abre gráfico para salvar imagem
func save_img():
	$FileSaveGraf.popup()

#Converte dados da viewport em imagem e salva como png
