...
#Associação de sinais relevantes entre nodes diferentes ou com este
func _ready():
...
#Cliente se conectou ao websocket
...
func _on_WebSocketsServer_disconnected():
	...
	#Muda para modo de teclado caso esteja no modo de editor
	if play_or_edit == 1:
		keyboard_mode = true
	#Recarrega jogo
	else:
...
func _on_WebSocketsServer_new_accel_data(data):
	...
	Game.tick(json_res)
	#caso esteja calibrando envia dados para o MenuConfig
	if calib == true:
		MenuConfig.json_res = json_res
	#Salva valor do úmito ângulo antes de perder a conexão
	last_ang = json_res

func _process(delta):
	#Permite jogar o jogo utilizando as teclas do teclado
	if keyboard_mode:
...
#Está calibrando ou parou de calibrar o jogo
func _calib_change():
	calib = !calib

#Encerra teste do Editor
...
#Salvar dados do paciente e fechar jogo
func _encerrar_sessao():
	...
	#Caso já exista Arquivo com preferências dos paciente le dados
	if file.file_exists(fname):
		file.open(fname,File.READ)
		data = parse_json(file.get_as_text())
		file.close()
	
	#caso não exista arquivo de paciente ou não tenha dados
	if data == null:
	...
	#Salva dados com preferências do jogador
	file.open(fname,File.WRITE)
	file.store_string(JSON.print(data," "))
	file.close()
	
	#Salvar histórico de desempenho do paciente
...
#Registra que está usando editor
func _abriu_editor():
	play_or_edit = 1

#Registra que está usando jogo
func _abriu_jogo():
	play_or_edit = 0

#Registra que não está selecionado jogo ou editor
