...
func _process(delta):
	...
	if calib and json_res != null:
		...
		if calib:
			...
			angulo = round_place((ang_max-ang_min)*angulo/180+ ang_min,0)
...
func calib():
	...
	if calib_max_min == 1:
		...
		if float(txtAngMax.text) < float(txtAngMin.text):
			txtAngMin.text = txtAngMax.text
	elif calib_max_min == 2:
		txtAngMin.text = $angulo_calib.text
		if float(txtAngMax.text) < float(txtAngMin.text):
			txtAngMax.text = txtAngMin.text 
