...
onready var listAuxVisual = $ListAuxiliosVisuais
...
var auxilioSprite = preload("res://scenes/AuxilioVisual/AuxSprite.tscn")
...
func spawn_enemy():
	...
	var inst_auxilio = auxilioSprite.instance()
	listAuxVisual.add_child(inst_auxilio)
	inst_auxilio.global_translation = $SpawnObj/SpawnAngulo/SpawnPoint/AuxSprite.global_translation
	inst_auxilio.rotation.z = $SpawnObj/SpawnAngulo/SpawnPoint/AuxSprite.global_rotation.z
	
	
	obs_ativos[count_obstaculo] = [Vector3(0,0,0),angulo_desejado,inst_auxilio]
...
func Update_Grafico():
	#pega ângulo atual
	graf_ang_sword = rad2deg(sword.rotation.z)#(ang_max-ang_min)*/180 +ang_min
	print(graf_ang_sword)
	obs_ideal()
	var values
...
func get_desempenho():
	...
	var historicocsv = "tempo,paciente ângulo,desejado ângulo,Acertos,Pontuação,Pontuação Máxima\n"
...
func Registro_historico():
	...
	var apend
	if Ponto_ou_Erro == 1:
		apend = [time_now,graf_ang_sword,graf_ang_obj,"Acerto",points, possible_points]
		Ponto_ou_Erro = 0
	elif Ponto_ou_Erro == 2:
		apend = [time_now,graf_ang_sword,graf_ang_obj,"Erro", points, possible_points]
		Ponto_ou_Erro = 0
	else:
		apend = [time_now,graf_ang_sword,graf_ang_obj,"-",points, possible_points]
